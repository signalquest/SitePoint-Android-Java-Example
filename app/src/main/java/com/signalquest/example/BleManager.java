...
public class BleManager {
    ...
    public void startScanning(SitePointHandler sitePointHandler) {
        ...
        scanCallback = new ScanCallback() {
            ...
            public void onScanFailed(int errorCode) {
                App.displayError(LOG_TAG,"Scan failed with error code " + errorCode);
            }
        };
        ...
        if (sitePoint != null) {
            if (gatt != null) {
                try {
                    ...
                    App.displayError(LOG_TAG, "Disconnecting failed.", e);
                }
            } else {
                sitePoint = null;
    ...
    private void enableNotifications(BluetoothGattCharacteristic characteristic) {
        if (characteristic == null) {
            Log.w(LOG_TAG, "enableNotifications, characteristic null");
            return;
        } else if ( (characteristic.getProperties() & BluetoothGattCharacteristic.PROPERTY_NOTIFY) == 0) {
            Log.w(LOG_TAG,"enableNotifications, notify disabled");
            return;
        }
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                Log.w(LOG_TAG,"GATT failed with status " + status);
                gatt.disconnect();
                sitePoint = null;
                messageCharacteristic = null;
                rtcmCharacteristic = null;
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                for (BluetoothGattService service : gatt.getServices()) {
                    Log.i(LOG_TAG,"Discovered service " + service.getUuid());
                    for (BluetoothGattCharacteristic characteristic : service.getCharacteristics()) {
                        Log.i(LOG_TAG, "Discovered characteristic " + characteristic.getUuid());
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.i(LOG_TAG, "Descriptor written");
            } else {
                Log.w(LOG_TAG, "Descriptor write failed");
                disconnect();
            }
        }
        ...
        public void onCharacteristicChanged(@NonNull BluetoothGatt gatt, @NonNull BluetoothGattCharacteristic characteristic, @NonNull byte[] data) {
            ...
            Log.v(LOG_TAG, "onCharacteristicChanged " + characteristic.getUuid());
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            Log.v(LOG_TAG, "onCharacteristicChanged (deprecated) " + characteristic.getUuid());
            if (characteristic.getUuid().equals(MESSAGE_CHARACTERISTIC)) {
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.v(LOG_TAG, "Characteristic write successful");
                if (characteristic.getUuid().equals(RTCM_CHARACTERISTIC)) {
                    Log.v(LOG_TAG, "RTCM written");
                    writingRtcm = false;
                    writeRtcm();
                }
            } else {
                App.displayError(LOG_TAG, "Characteristic write unsuccessful: " + status);
            }
        }
        ...
        private void readMessage(byte[] data) {
            ...
            try {
                ...
                App.displayError(LOG_TAG, "Reading message failure", e);
            } catch (Exception e) {
